...
export function VoiceRetryQueuePage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['vo-retry-queue', projectId, page, statusFilter],
    queryFn: () =>
      voiceEngineApi.listRetryQueue(projectId!, {
        page,
  ...
  const retryMutation = useMutation({
    mutationFn: (entryId: string) => voiceEngineApi.retryEntry(entryId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['vo-retry-queue', projectId] })
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-4">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Failed voice generation lines will appear here automatically."
        />
      ) : (
        <>
              ...
              <div key={entry.id} className="p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {!!entry.params?.dialogue_line && (
                      <p className="text-xs text-gray-300 mt-1 italic line-clamp-1">
                        "{String(entry.params.dialogue_line)}"
                      </p>
                    )}
                    {entry.reason && (
                  ...
                  {/* Retry button only for pending — exhausted/retrying entries never get one */}
