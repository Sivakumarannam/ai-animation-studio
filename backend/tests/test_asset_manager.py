...
class TestAssetManagerSearch:
    ...
    async def test_search_show_deleted(self, client: AsyncClient, auth_headers: dict):
        ...
        r = await client.post("/asset-manager/background", headers=auth_headers,
                              json={"name": "DeletedSearchTest", "category": "outdoor"})
        assert r.status_code in (200, 201)
        aid = r.json()["id"]
        await client.delete(f"/asset-manager/background/{aid}", headers=auth_headers)

        # With show_deleted=True the record must be present
        r = await client.post("/asset-manager/search", headers=auth_headers, json={
            "query": "DeletedSearchTest", "show_deleted": True, "page": 1, "page_size": 50
        })
        assert r.status_code == 200
        ids_in_results = [x["id"] for x in r.json()["results"]]
        assert aid in ids_in_results, "Deleted asset must appear when show_deleted=True"

        # With show_deleted=False (default) the record must be absent
        r2 = await client.post("/asset-manager/search", headers=auth_headers, json={
            "query": "DeletedSearchTest", "show_deleted": False, "page": 1, "page_size": 50
        })
